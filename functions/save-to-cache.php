<?php
	...
	if($fullFeed->error){
		...
		$sizedURL = snazzy_ff_addToCache($sizedURL, "1000", $fullDir);
		echo "
		<script>window.onload = function() {(function(d, s, id) { var js, fjs = d.getElementsByTagName(s)[0]; if (d.getElementById(id)) return; js = d.createElement(s); js.id = id; js.src = '//connect.facebook.net/en_EN/all.js#xfbml=1&status=0'; fjs.parentNode.insertBefore(js, fjs); }(document, 'script', 'facebook-jssdk'));};</script>
			<div class='sff-the-cover' style='background-image: url(\"" . $sizedURL . "\");'>
				...
				<div id='fb-root'></div>
				<div class='fb-like' data-href='https://www.facebook.com/" . $facebookID . "' data-layout='button' data-action='like' data-size='small' data-show-faces='false' data-share='true'></div>
			</div>
		";
		foreach ($fullFeed->data as $feedData ) {
			...
			if(property_exists($feedData, "object_id")){
			...
			<div class='sff-feed-item'>
				...
				<div class='sff-social-buttons'>
					<div class='sff-likes'>
						<img src='" . $pluginsURL . "/images/facebook-like.svg' id='facebook-like-icon'>
						" . $likes . "
					</div>
					<div class='sff-shares'>
						<img src='" . $pluginsURL . "/images/facebook-share.svg' id='facebook-share-icon'>
						" .$shares."
					</div>
				</div>
			</div>
